...
class Player {
    ...
    jump() {
        if (this.grounded) {
            ...
            jumpSound.play().catch(e => console.log("Audio play interrupted", e));
        }
    }
}
...
function initGame() {
    ...
    obstacles = [];
    clouds = [
        new Cloud(canvas.width / 2, canvas.height / 4), 
        new Cloud(canvas.width, canvas.height / 3)
    ];
    stars = [];
    ...
    bgMusic.play().catch(e => console.log("Muzyka wymaga interakcji", e));
...
function animate(currentTime) {
    ...
    spawnTimer += dt;
    const spawnInterval = Math.max(0.8, 1.8 - (gameSpeed / 1000)); 
    if (spawnTimer >= spawnInterval) {
        spawnObstacle();
        spawnTimer = 0;
    }
    
    // Bezpieczna pętla usuwająca przeszkody od końca tablicy, zapobiegająca glitchom kolizji
    ...
    if (scoreTimer >= 0.05) { // Szybkie naliczanie wizualne
        ...
        if (score > 0 && score % 1000 === 0) {
            ...
            milestoneSound.play().catch(e => console.log("Audio error", e));
